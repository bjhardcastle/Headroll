...
for fidx = 1:length(flies)
    for cidx = 1:5
        % abs resp (gain is then subtracted from 1)
...
%         hold off
        ...
        %%% as used for FFTs. 
        %%% NEW METHOD: use calc_gain_CIRC, with stimperiod of
        %%% peak freq in window
        ...
        steplength = 1 + (floor(length(datALIN)/2)+floor(0.5*xwin_size) - xwin_size)/underlap;
...
%             [~,ph] = max(cor_u( 1: ceil(length(lu)) ) );
%             
%             lag_u(stepidx) = lu(ph);
%             corr_u(stepidx,:) = cor_u;
            
            % NEW METHOD:
            stim = datALIN(steprange)';
            resp = datALOUT(steprange)';
            stimperiod = floor(sampling_rate/flocsu(stepidx));
            calc_gain_phase_circ;
            lag_u(stepidx) = CL_phase;
            corr_u(stepidx,:) = CL_gain;
...
%             [~,ph] = max(cor_d( 1: ceil(length(ld)) ) );
% %             lag_d(stepidx) = ld(ph);
%             corr_d(stepidx,:) = cor_d;
            stim = datALIN(steprange_d)';
            resp = datALOUT(steprange_d)';
            stimperiod = floor(sampling_rate/flocsd(stepidx));
            calc_gain_phase_circ;
            lag_d(stepidx) = CL_phase;
            corr_d(stepidx,:) = CL_gain;
...
%         lag_d = fliplr(lag_d);
        ...
        G_up(cidx,fidx,:) = corr_u; 
        
        lags_down(cidx,fidx,:) = lag_d;
        G_down(cidx,fidx,:) = corr_d;
        
        
    end

end
%     lags_down = flip(lags_down,3);
% 
%     G_down = flip(G_down);
%% plot mean gains and lags by condition
Tplot_chirp_gains
...
%%
run('C:\Users\Ben\Dropbox\Work\Thesis\Thesis_data\Blowflies\Sines\Fran2016\Fly Gaze Data\flystab\varanalysis_main.m')
% run analysis, then run plots..
run('C:\Users\Ben\Dropbox\Work\Thesis\Thesis_data\Blowflies\Sines\Fran2016\Fly Gaze Data\flystab\Tchirp_sine_compare.m')
