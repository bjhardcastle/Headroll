...
wind_cond{1} =('wind');
wind_cond{2} =('no_wind');
flies = [8,9,10,11,12,13,14,15,16];
% fly 16 is a good 'example' for chirp response
...
if exist(data_path)
    ...
    resp_files = dir([data_path,'*',data_type{1},'*.mat']);
    stim_files = dir([data_path,'*',data_type{2},'*.mat']);
...
%     for file_counter = 1:length(files)
         ...
         load([data_path,resp_files(files(file_counter)).name])
...
%         file_number = strrep(file_number,[data_type, '.mat'],'');
        ...
        title(['C',num2str(c),' ',resp_files(file_counter).name,''],'Interpreter','none');
        subplot(3,4,5:6)
        ...
        if SHOWSTIM_FLAG == 1
            % Load Stim
            load([data_path,stim_files(files(file_counter)).name])
            % Plot stim
            subplot(3,4,3:4)
            plot(data(:,6),data(:,1),'b',data(:,6),data(:,2),'r'); title('L (blue) and R (red) roll');
