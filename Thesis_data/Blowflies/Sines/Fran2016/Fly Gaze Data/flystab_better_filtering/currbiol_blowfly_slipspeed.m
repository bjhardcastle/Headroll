...
%                     end
                    ...
                    xvalues = linspace(0,2*2*pi*f2req(p)*30,50); %2 times the maximum angular speed of stim
                    
                    
                    [N,edges] = histcounts(resp_vel,xvalues,'Normalization','pdf');
                    result(fly,cond,p2).x_resp=(edges(1:end-1)+edges(2:end))/2;
                    result(fly,cond,p2).n_resp=N;
                    result(fly,cond,p2).mean_resp=median(resp_vel);
                    [N,edges] = histcounts(stim_vel,xvalues,'Normalization','pdf');
                    result(fly,cond,p2).x_stim=(edges(1:end-1)+edges(2:end))/2;
                    result(fly,cond,p2).n_stim=N;
                    result(fly,cond,p2).mean_stim=median(stim_vel);
...
%       export_fig(['Z:\Ben\Current Biology\Figures\calliphora_slipspeedSTANDARDFREQS'], '-openGL','-r660')
...
for p = num_freqs:num_freqs+n2um_freqs-1
...
totfreq = [0.06 0.1 0.3 0.6 1 3 6 10,15,20,25];
numtotfreq = 11;

figure(667)
lineprops.linewidth = 1.5;
lineprops.col = {color_mat{6}};
% l1=mseb(totfreq(1:numtotfreq), mean_s(1:numtotfreq),std_s(1:numtotfreq)./sqrt(9),lineprops,1)
l1=errorbar(totfreq(1:numtotfreq), mean_s(1:numtotfreq),std_s(1:numtotfreq)./sqrt(9),'LineWidth',lineprops.linewidth, 'Color',color_mat{6})

hold on

lineprops.col = {color_mat{4}};
% l3=mseb(totfreq(1:numtotfreq), mean_2(1:numtotfreq),std_2(1:numtotfreq)./sqrt(9),lineprops,1)
l3=errorbar(totfreq(1:numtotfreq), mean_2(1:numtotfreq),std_2(1:numtotfreq)./sqrt(9),'LineWidth',lineprops.linewidth, 'Color',color_mat{4})

lineprops.col = {color_mat{2}};
% l2=mseb(totfreq(1:numtotfreq), mean_1(1:numtotfreq),std_1(1:numtotfreq)./sqrt(9),lineprops,1)
l2=errorbar(totfreq(1:numtotfreq), mean_1(1:numtotfreq),std_1(1:numtotfreq)./sqrt(9),'LineWidth',lineprops.linewidth, 'Color',color_mat{2})
...
%    export_fig(['Z:\Ben\Current Biology\Figures\slipspeedsMAX_calliphora'], '-painters','-r660')
%     export_fig(['C:\Users\Ben\Desktop\CurrBiol\Figures\slipspeedsMAX_calliphora'], '-painters','-r660')
