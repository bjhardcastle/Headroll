...
for fly = 1:length(fly_array)
    for cond = 1: length(cond_array)
        ...
        for freq = 1:num_freqs
            ...
            F_AdB_mean(fly,cond,freq) = mean(20*log10(abs(step_vector_F)));
            F_AdB_std(fly,cond,freq) = std(20*log10(abs(step_vector_F)));
            ...
            G_AdB_mean(fly,cond,freq) = mean(20*log10(abs(step_vector_G)));
            G_AdB_std(fly,cond,freq) = std(20*log10(abs(step_vector_G)));
