...
freqs = [0.011, 0.03, 0.1, 0.3, 1 ,3, 6, 10, 15];   % exact stimulation frequencies
amps = [40];               % stimulation amplitudes
conds = [1];                   % "names" of conditions

framerates = [50,50,50,60,250,500,500,1000,1000];  % camera rates used for each of the freqs
...
p2 = '\Responce\';           % Suffix of flynumber in path
p3 = '\Stimulus\';                    % ...
...
for fly_nr = 1:num_flies
    ...
    stim_path = [p1,int2str(fly),p3]; % path of mat-files
    ...
    for freq_nr = 1:num_freqs
        ...
        for amp_nr = 1:num_amps
            ...
            for cond_nr = 1:num_conds
                ...
                stim_file =['Fly',int2str(fly),'_',num2str(freq),'_',int2str(amp),'_C',int2str(cond),'_',f2,'stim.mat'];
            if exist([stim_path,stim_file]) 
                ...
                resp_file =['Fly',int2str(fly),'_',num2str(freq),'_',int2str(amp),'_C',int2str(cond),'_',f2,'resp.mat'];
                ...
                resp =  datastim(:,3)' - dataresp(:,3)';   % Extract response timeseries
