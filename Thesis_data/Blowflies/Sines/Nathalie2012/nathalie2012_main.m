...
flies = [2:12,14];                   % "names" of flies to be included
...
resp_gain_mean = NaN(num_flies,num_freqs,num_amps,num_conds);
resp_phase_mean = NaN(num_flies,num_freqs,num_amps,num_conds);
resp_gain_std = NaN(num_flies,num_freqs,num_amps,num_conds);
resp_phase_std = NaN(num_flies,num_freqs,num_amps,num_conds);

offset_mean = NaN(num_flies,num_freqs,num_amps,num_conds);
offset_std = NaN(num_flies,num_freqs,num_amps,num_conds);
...
for fly_nr = 1:num_flies
    ...
    fly = flies(fly_nr);
    resp_path = [p01,int2str(cond),'\',p1,int2str(fly),p3]; % path of mat-files
    stim_path = [p01,int2str(cond),'\',p1,int2str(fly),p2]; % path of mat-files
    ...
    for freq_nr = 1:num_freqs
        ...
        for amp_nr = 1:num_amps
            ...
            stim_file =['Fly',int2str(fly),'_',int2str(freq),'_',int2str(amp),'_C2_',f2,'stim.mat'];
         ...
         if exist([stim_path,stim_file]) 
            ...
            stim = datastim(:,3)';       % Extract stimulus timeseries
                ...
                resp = stim - resp; %Relative resp: Thorax roll - head roll
...
for amp_nr = 1:num_amps
        ...
        offm1 = nanmean(offsets,1);
        offs1 = nanstd(offsets,1)/sqrt(num_flies);
        
        offstds = squeeze(offset_std(:,:,amp_nr,1));
        ostdm1 = nanmean(offstds,1)./offm1;
        ostds1 = (nanstd(offstds,1)./offm1)/sqrt(num_flies);
        
        
        
        offsets = squeeze(offset_mean(:,:,amp_nr,2));
        offm2 = nanmean(offsets,1);
        offs2 = nanstd(offsets,1)/sqrt(num_flies);
        
        offstds = squeeze(offset_std(:,:,amp_nr,2));
        ostdm2 = nanmean(offstds,1)./offm2;
        ostds2 = (nanstd(offstds,1)./offm2)/sqrt(num_flies);        
