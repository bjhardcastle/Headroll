...
%         figure(91)
%         subplot(4,lencond,(fidx-1)*lencond + cidx)
...
%         hold off
        ...
        gains_down(cidx,fidx,:) = 1-gainsd;
        freqs_down(cidx,fidx,:) = flocsd;
        gains_up(cidx,fidx,:) = 1-gainsu;
...
err_gu = squeeze(std(gains_up,[],2)/sqrt(4));
...
err_gd = squeeze(std(gains_down,[],2)/sqrt(4));
...
err_lu = squeeze(std(lags_up,[],2)/sqrt(4));
err_ld = squeeze(std(lags_down,[],2)/sqrt(4));
subplot(1,2,1)
...
avg_lag(:,1) = mean(mean_lu(:,10:end-1,:),2);
avg_lag(:,2) = mean(mean_ld(:,10:end-1,:),2);
...
err_gu = squeeze(std(mag_up,[],2)/sqrt(4));
% plot(mean_fu',mean_gu')
errorbar(mean_fu',mean_gu',err_gu')
axis([0 20 0 0.8])
xlabel('freq, Hz')
...
err_gd = squeeze(std(mag_down,[],2)/sqrt(4));
% plot(mean_fd',mean_gd')
errorbar(mean_fd',mean_gd',err_gd')
axis([0 20 0 0.8])
xlabel('freq, Hz')
...
err_phu = squeeze(std(ph_up,[],2)/sqrt(4));
...
err_phd = squeeze(std(ph_down,[],2)/sqrt(4));
