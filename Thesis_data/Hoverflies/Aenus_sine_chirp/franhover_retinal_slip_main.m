        ...
        for cond = 1:length(cond_array)
            ...
            for p = 1:num_freqs
                ...
                if exist([data_path resp_fname]) && exist([data_path stim_fname])
                    ...
                    result(fly,cond,p).headvel = (resp_diff(2:length(resp_diff))*Fs);
                    result(fly,cond,p).thoraxvel = (stim_diff(2:length(resp_diff))*Fs);
                    result(fly,cond,p).Fs = fps;
                    ...
                    result(fly,cond,p).headvel = nan(1,length(resp_diff(2:length(resp_diff))*Fs));
                    result(fly,cond,p).thoraxvel = nan(1,length(stim_diff(2:length(resp_diff))*Fs));
                    result(fly,cond,p).Fs = NaN;
...
%       export_fig(['C:\Users\Ben\Dropbox\Work\Thesis\Chapter_3\Figures\aeneus_slipspeedSTANDARDFREQS'], '-openGL','-r660')
...
%      export_fig(['C:\Users\Ben\Dropbox\Work\Thesis\Chapter_3\Figures\aeneus_slipspeedsMAX'], '-openGL','-r660')
