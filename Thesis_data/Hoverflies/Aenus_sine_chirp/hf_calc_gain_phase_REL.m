...
for step = 2:num_steps-1
    ...
    for k = 0:L+0.5*L-1
        ...
        resp_win = resp((step-1)*stimperiod+k+1:(step-1)*stimperiod+L+k); % One cycle of resp, shifted by k samples w.r.t stim_win
        resp_off(step) = mean(resp_win);
        resp_win = resp_win - resp_off(step);                               % Remove offset
%         resp_win = (stim_win - resp_win);   % comment                                % Relative head roll, with sign inverted. (+ve direction = same dir as stimulus)
...
%     [,~] = max(gcorr_est);
    ...
    gain(step) = (stim_win - (resp_win-mean(resp_win)))/ stim_win  ;  
