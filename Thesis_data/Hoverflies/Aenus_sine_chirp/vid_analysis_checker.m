function [] = vid_analysis_checker()

clear all
close all

% Kat Reid 2015
% Plots the L/R, mean head roll, score, and outputs minimum score.
% Possible to set file path, minimum mean score, resp/stim, file prefeix
% Uses subfunction natsort, which has been integrated into the bottom of the
% code

%%%%%VARIABLES%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% directory file path information, folder containing matlab files
project_path = 'Z:\Ben\Hoverflies_2015\ben\3_aug Chirp, need conditions from lab book\';
% Are stim files in the same folder? Plot these with FLAG = 1
SHOWSTIM_FLAG = 1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% files = [1:27]
data_type{1} = 'resp'; %resp or stim or blank for all files
data_type{2} = 'stim';

% optional tidying of filename
% file_prefix = '3_Aug_Cam_v211_CineF';

% set(0,'DefaultFigureWindowStyle','docked') %docks the figures
 
if exist(project_path)
    
    resp_files = dir([project_path,'*',data_type{1},'*.mat']);
    stim_files = dir([project_path,'*',data_type{2},'*.mat']);
    files = 1:length(resp_files);
    
    %%sort file names naturally:
    [~,index] = natsort({resp_files.name}.'); 
    resp_files = resp_files(index); clear index; 
    [~,index] = natsort({stim_files.name}.'); 
    stim_files = stim_files(index); clear index; 
    minimum_scores = [files(:),zeros(size(files(:)))];
    
    
   figure
   
   file_counter = 1; last_file_reached = 0;
   while last_file_reached == 0
%     for file_counter = 1:length(files)
        
         load([project_path,resp_files(files(file_counter)).name])
        
        %optionally produces the file number for use in title plots etc
%         file_number = [resp_files(files(file_counter)).name]; 
%         file_number = strrep(file_number, file_prefix, ''); 
%         file_number = strrep(file_number,[data_type, '.mat'],'');
        
        % Exception for one or other files not existing,
        % blank plots
        
        % Plot resp 
        subplot(3,4,1:2)
        plot(data(:,6),data(:,1),'b',data(:,6),data(:,2),'r'); title('L (blue) and R (red) roll');
        axis([0 length(data(:,6)) -40 40]);
        title([resp_files(file_counter).name],'Interpreter','none');
        subplot(3,4,5:6)
        plot(data(:,3)); title('Average roll'); 
        axis([0 length(data(:,3)) -40 40]);
        subplot(3,4,9:10)
        plot(data(:,6),data(:,4)); title('Score');  xlabel('Frame');
        axis([0 length(data(:,4)) 0 1000]);
        
        if SHOWSTIM_FLAG == 1
            % Load Stim
            load([project_path,stim_files(files(file_counter)).name])
            % Plot stim
            subplot(3,4,3:4)
            plot(data(:,6),data(:,1),'b',data(:,6),data(:,2),'r'); title('L (blue) and R (red) roll');
            title([stim_files(file_counter).name],'Interpreter','none');
            subplot(3,4,7:8)
            plot(data(:,3)); title('Average roll');
            subplot(3,4,11:12)
            plot(data(:,4)); title('Score');  xlabel('Frame');
            axis([0 length(data(:,4)) 0 1000]);
        else
            subplot(3,4,3:4)
            title('Mouse click to reverse.')
        end
        
        min_score = min(data,[],1);
        minimum_scores(file_counter,2) = min_score(4);

        min_score(4);
%         
%         if min_score(4) < minimum_mean_score
%             fprintf(file_number);
%             fprintf('   %.0f    !too low!\n',min_score(4));
%             %         else
%             %             fprintf(file_number);
...
%         end
        ...
        if file_counter == length(files) +1
        last_file_reached = 1;
        end
    end
...
%%subfunction to sort the file names naturally, e.g. so 10 is after 1
