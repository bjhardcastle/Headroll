...
recalculate = 0;                % calc gain and phase data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if recalculate == 1
    
clear all
path = 'C:\Users\Ben\Dropbox\Work\Robberfly2012\';
...
resp_gain_mean = zeros(2,num_flies,num_freqs,num_amps,num_conds);
resp_phase_mean = zeros(2,num_flies,num_freqs,num_amps,num_conds);
resp_gain_std = zeros(2,num_flies,num_freqs,num_amps,num_conds);
resp_phase_std = zeros(2,num_flies,num_freqs,num_amps,num_conds);
...
for cond_nr = 1:num_conds
  ...
  for fly_nr = 1:length(flies)
    ...
    stim_path = [p1,int2str(fly),p2]; % path of mat-files
        ...
        for amp_nr = 1:num_amps;
           ...
           for repeat = 1:repeats           
               ...
               if exist([resp_path,resp_file],'file')
                ...
                resp = datastim(fstart:fstop,3)' - dataresp(fstart:fstop,3)';   % Extract relative response timeseries ( zero amplitude = no compensation )
...
%                 figure, plot(time,stim,time,resp), title(['fly ',num2str(fly),', freq ',num2str(freq),', flight ',num2str(flight),' repeat ',num2str(repeat),', gain ',num2str(CL_gain),''])
                ...
                if repeat ~=1                       %                   
                resp_gain_mean(group,fly_nr,freq_nr,amp_nr,cond_nr) = 0.5*(resp_gain_mean(group,fly_nr,freq_nr,amp_nr,cond_nr) + nanmean(partial_resp_gain_mean));          
                resp_phase_mean(group,fly_nr,freq_nr,amp_nr,cond_nr) = 0.5*(resp_phase_mean(group,fly_nr,freq_nr,amp_nr,cond_nr) + nanmean(partial_resp_phase_mean));                       
                else                     
...
h1 = errorbar(round(freqs),mean(ta1,2),st1,'b');
...
h2 = errorbar(round(freqs),mean(ta2,2),st2,'r'); 
...
p1 = errorbar(round(freqs),mean(tb1,2),pt1,'b'); 
...
p2 = errorbar(round(freqs),mean(tb2,2),pt2,'r'); 
